...
- **Markdown** — full text. Other prose formats index too, via `doc_exts` — set `doc_exts = [".md", ".rst", ".adoc"]` for a repo holding reStructuredText or AsciiDoc (Python's and git's own docs, for instance). It defaults to `[".md"]`, so **without it none of those files are indexed at all** — silently, since they are simply never walked. Headings and titles are detected across all three (Markdown/AsciiDoc `#`/`==` prefixes and reStructuredText/setext underlines), so `.rst`/`.adoc` get the same per-section chunking and display titles `.md` gets. Turning `doc_exts` on for an already-indexed repo only re-chunks files as they change — run `repolens index --rebuild` once to apply it to everything. (Two small notes: file matching is by lowercased extension, so a bare `.md` dotfile is skipped and `README.MD` is included; and non-UTF-8 bytes in a doc are dropped on read rather than failing the index.)
