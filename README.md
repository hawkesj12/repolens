...
- **Markdown** — full text. Other prose formats index too, via `doc_exts` — set `doc_exts = [".md", ".rst", ".adoc"]` for a repo holding reStructuredText or AsciiDoc (Python's and git's own docs, for instance). It defaults to `[".md"]`, so **without it none of those files are indexed at all** — silently, since they are simply never walked.
